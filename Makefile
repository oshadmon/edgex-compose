...
.PHONY: help portainer portainer-down pull run pull-ui run-ui down-ui down clean get-token clean-network
...
# This tool now only supports compose V2, aka "docker compose" as it has replaced the old docker-compose tool.
...
# Define additional phony targets for all options to enable support for tab-completion in the shell
...
clean-network:
	...
	${DOCKER_COMPOSE} -p edgex -f docker-compose.yml -f docker-compose-with-app-sample.yml network rm edgex_edgex-network
